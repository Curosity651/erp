 ...
 * 自定义入库单数据传输对象
 * <p>
 * 用于不经过「采购 → 物流 → 入库」完整链路的入库（如调拨、退货归仓、期初等）。 与采购入库单共用
 * {@code wms_purchase_inbound_order} 表，{@code source_type=MANUAL}，无关联物流单/采购单。
 * </p>
 *
 * @author erp
 */
@Data
@Schema(title = "自定义入库单数据传输对象")
public class ManualInboundDTO {
    ...
    @Schema(title = "入库明细列表")
    private List<ManualInboundItemDTO> items;
