...
/**
 * Render Footer tabList In purchase-order to be compatible with the old version of the PageHeader basically
 * all the functions are implemented.
 *
 * 优先级：pageHeaderFooter slot > tabList props > null
 */
const renderFooter = (
  props: Omit<PageContainerProps & { prefixedClassName: string }, 'title'>,
  slots?: Record<string, any>
) => {
  // 优先使用 pageHeaderFooter slot（完全自定义 PageHeader footer）
  if (slots?.pageHeaderFooter) {
    return slots.pageHeaderFooter()
  }

  // 其次使用 tabList 配置渲染 tabs
...
const ProPageHeader = defineComponent({
  ...
  setup(props, { slots }) {
    ...
    // 当 pageHeaderRender=false 时，检查是否有 customHeader slot
    if (props.pageHeaderRender === false) {
      // 如果有 customHeader slot，渲染自定义头部
      if (slots.customHeader) {
        return () => <div class={`${props.prefixedClassName}-warp`}>{slots.customHeader()}</div>
      }
      // 否则不渲染任何内容
      return () => null
    }
...
/**
 * PageHeader 支持的 slots
 * 注意：footer slot 专用于 FooterToolbar（页面底部固定操作栏），不传递给 PageHeader
 * 如需自定义 PageHeader 底部区域（覆盖 tabs），使用 pageHeaderFooter slot
 */
const pageHeaderSlot = [
  ...
  'tags',
  'extra',
  'pageHeaderFooter', // 用于完全自定义 PageHeader footer 区域（覆盖 tabList）
  'customHeader' // 用于完全自定义页面头部（当 pageHeaderRender=false 时使用）
]
  ...
  setup(props, { attrs, slots }) {
    ...
    const containerClassName = computed(() => [
      ...
      attrs.class,
      {
        ...
        [`${prefixCls}-page-container-with-footer`]: props.footer || slots.footer
      }
    ])
    ...
    return () => {
      const pageHeaderDom = (
        ...
        >
          {reactiveOmit(slots, 'default', 'loading', 'footer')}
        </ProPageHeader>
      )
      ...
      return (
        <div style={attrs.style as CSSProperties} class={containerClassName.value}>
          ...
          {(props.footer || slots.footer) && (
            <FooterToolbar prefixCls={prefixCls}>{slots.footer?.() || props.footer}</FooterToolbar>
          )}
        </div>
      )
    }
