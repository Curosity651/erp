...
import { pageUsers, deleteUser, updateUserAvatarPath, updateUserStatus } from '@/api/system/user'
...
/* 修改头像 - 使用 OSS 直传 */
const uploadAvatarProcessor = async (fileObj: FileObject, record: SysUserPageVO) => {
  try {
    // 创建 File 对象用于上传
    const file = new File([fileObj.data], fileObj.name, { type: fileObj.data.type })

    // 使用带缓存的上传方法
    const uploadResult = await uploadToOSSWithCache(file, {
      addTimestamp: true,
      sanitizeFileName: true,
      filePrefix: `avatar_${record.userId}`,
      timeFormat: 'timestamp',
      folder: 'avatars' // 将头像文件存储在 avatars 文件夹中
    })

    // 更新用户头像记录 - 传递文件路径给后端
    const avatarPath = uploadResult.objectKey
    const updateRes = await updateUserAvatarPath(record.userId, avatarPath)

    // 更新表格头像显示
    record.avatar = avatarPath
