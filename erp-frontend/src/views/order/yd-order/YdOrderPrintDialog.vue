...
import { printYdLabels } from '@/api/order/yd-order'
import type { LabelBatchVO } from '@/api/order/yd-order/types'
...
function reasonOf(row: any) {
  const isYandex = (row?.platform || '').toLowerCase() === 'yandex'
  if (!isYandex) return '非 Yandex 平台'
...
async function onOk() {
  if (state.step === 'preview') {
    ...
    state.loading = true
    try {
      const resp = await printYdLabels(ids)
      state.result = resp?.data as LabelBatchVO
      state.step = 'result'
      emit('printed', state.result)
    } catch (e: any) {
      message.error(e?.message || '打印面单失败')
    } finally {
      state.loading = false
    }
    return
