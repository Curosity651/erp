...
import { printOzonLabels } from '@/api/order/ozon-order'
import type { LabelBatchVO } from '@/api/order/ozon-order/types'
...
function canPrint(row: any) {
  const isOzon = (row?.platform || '').toLowerCase() === 'ozon'
  if (!isOzon) return false
  const isFbs = row?.fulfillmentType === 'FBS'
  if (!isFbs) return false
...
function reasonOf(row: any) {
  const isOzon = (row?.platform || '').toLowerCase() === 'ozon'
  if (!isOzon) return '非 Ozon 平台'
  if (row?.fulfillmentType !== 'FBS') return 'FBO 订单不支持打印'
...
async function onOk() {
  if (state.step === 'preview') {
    ...
    state.loading = true
    try {
      const resp = await printOzonLabels(ids)
