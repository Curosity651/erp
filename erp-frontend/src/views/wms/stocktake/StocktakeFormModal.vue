  ...
  >
    <a-form ref="formRef" :model="form" layout="vertical">
      ...
      <a-alert v-if="form.stocktakeMode === 'FULL'" type="info" show-icon>
        <template #message>全仓盘点将为仓库内每个逻辑库位生成任务，包括当前账面为空的库位。</template>
      </a-alert>
...
import type { StocktakeDTO } from '@/api/wms/stocktake/types'
import { listLocations } from '@/api/wms/location-mgmt'
import type { WmsLocation } from '@/api/wms/location-mgmt/types'
import { isSuccess } from '@/api'
import {
  creatableStocktakeModes,
  type CreatableStocktakeMode
} from './stocktake-mode-policy'
...
const modeIcons = { FULL: EnvironmentOutlined, CYCLE: RetweetOutlined }
const modes = creatableStocktakeModes.map(mode => ({ ...mode, icon: modeIcons[mode.value] }))
...
function selectMode(mode: CreatableStocktakeMode) {
