<template>
  ...
  >
    <a-form :model="form" :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }">
      <a-form-item label="货主名称" required>
        <a-input v-model:value="form.tenantName" placeholder="公司名称" />
      </a-form-item>
      <a-form-item label="货主编码" required>
      ...
      <a-form-item label="管理员昵称">
        <a-input v-model:value="form.adminNickname" placeholder="默认取货主名称" />
      </a-form-item>
...
import { openErpTenant } from '@/api/tenant'
...
async function submit() {
  if (!form.tenantName || !form.tenantCode || !form.adminUsername || !form.adminPassword) {
    message.warning('货主名称/编码、管理员账号/密码不能为空')
    return
  }
  submitting.value = true
  try {
    const res = await openErpTenant(form)
    if (isSuccess(res)) {
