    ...
    <a-spin :spinning="loading">
      <template v-if="detail">
        ...
        <a-descriptions title="基本信息" :column="2" bordered size="small">
          <a-descriptions-item label="退货单号">{{ detail.inboundNo }}</a-descriptions-item>
          <a-descriptions-item label="单据状态">
            <inbound-status-badge :status="detail.orderStatus" />
          </a-descriptions-item>
          <a-descriptions-item label="退货类型">
            <a-tag
              v-if="detail.returnType"
              :color="CUSTOM_RETURN_TYPE_COLOR_MAP[detail.returnType]"
            >
              {{ CUSTOM_RETURN_TYPE_MAP[detail.returnType] || detail.returnType }}
            </a-tag>
            <span v-else>-</span>
          </a-descriptions-item>
          ...
          >
            <template #bodyCell="{ column, record }">
              ...
              <template v-else-if="column.key === 'expectedQuality'">
                <a-tag
                  v-if="record.expectedQuality"
                  :color="EXPECTED_QUALITY_COLOR_MAP[record.expectedQuality]"
                >
                  {{ EXPECTED_QUALITY_MAP[record.expectedQuality] || record.expectedQuality }}
                </a-tag>
                <span v-else>-</span>
              </template>
              <!-- 实收数量：草稿/已提交显示状态文字，已收货/已完成显示平台录入的数字 -->
...
import { getCustomReturnDetail, submitCustomReturn } from '@/api/wms/custom-return'
import type { CustomReturnDetailVO } from '@/api/wms/custom-return/types'
import {
  CUSTOM_RETURN_TYPE_MAP,
  CUSTOM_RETURN_TYPE_COLOR_MAP,
  EXPECTED_QUALITY_MAP,
  EXPECTED_QUALITY_COLOR_MAP
...
const detail = ref<CustomReturnDetailVO | null>(null)

const itemColumns = [
  { title: 'SKU信息', key: 'skuBrief', width: 220 },
  { title: '货品预判', key: 'expectedQuality', width: 100, align: 'center' },
  { title: '应退数量', dataIndex: 'expectedQuantity', width: 100, align: 'center' },
  { title: '实收数量', key: 'actualQuantity', width: 100, align: 'center' },
  { title: '差异数量', key: 'shortQuantity', width: 100, align: 'center' }
]

// 实收/差异列：草稿/已提交阶段尚无实收数据，显示状态文字；已收货/已完成显示平台录入的数字
...
const open = async (id: number) => {
  ...
  loading.value = true
  try {
    const result = await getCustomReturnDetail(id)
    if (isSuccess(result)) {
...
const handleEdit = () => {
  if (detail.value) {
    visible.value = false
    router.push(`/wms/custom-return/form/edit/${detail.value.id}`)
  }
}
...
const handleSubmit = () => {
  ...
  Modal.confirm({
    ...
    cancelText: '取消',
    onOk: () => {
      doRequest(submitCustomReturn(detail.value!.id), {
        successMessage: '提交成功',
        onSuccess: () => {
