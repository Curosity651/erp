  ...
  if (error.response) {
    ...
    switch (errorStatus) {
      ...
      default:
        error.resolved = true
        notification.error({
          message: rawI18nText('system.tip.server.error', '服务器处理失败!'),
          description:
            data?.message ||
            errorStatusText ||
            error.message ||
...
// 创建用于获取完整响应的 axios 实例（用于需要访问响应头的场景）
const rawAxiosInstance = axios.create({
  baseURL: import.meta.env.VITE_API_URL,
  timeout: import.meta.env.VITE_API_TIME_OUT
})

// 仅添加请求拦截器（保留 token、i18n 功能）
rawAxiosInstance.interceptors.request.use(onRequestFulfilled)

// 添加响应拦截器（返回完整响应，但保留错误处理）
rawAxiosInstance.interceptors.response.use(
  response => response, // 成功时返回完整响应
  onResponseRejected // 失败时统一错误处理
)

// 导出原始 axios 实例供特殊场景使用
export const rawAxios = rawAxiosInstance
