  ...
  setup(props, { emit, attrs }) {
    ...
    const addCachedComponentNames = (
      ...
      matchedRoutes.forEach(route => {
        ...
        if (componentName && route.meta.keepAlive !== false) {
          // 如果有自定义缓存策略，使用自定义缓存键
          if (currentRoute && route.meta?.cacheStrategy && route.meta?.cacheKey) {
            const customCacheKey = (route.meta.cacheKey as Function)(currentRoute)
            multiTabStore.addCachedComponent(customCacheKey)
          } else {
            multiTabStore.addCachedComponent(componentName)
          }
        }
      })
    }
    ...
    watch(
      ...
      () => {
        ...
        // 更新缓存的组件
        addCachedComponentNames(currentRoute.matched, currentRoute)
      },
      { immediate: true }
    )
    ...
    const handleTabEdit = (
      ...
      if (action === 'remove') {
        ...
        // 移除缓存
        const targetRoute = routeList[index]
        targetRoute.matched.forEach(route => {
          const componentName = route.components?.default.name
          if (componentName && componentName !== routerLayoutName) {
            // 如果有自定义缓存策略，删除自定义缓存键
            if (route.meta?.cacheStrategy && route.meta?.cacheKey) {
              const customCacheKey = (route.meta.cacheKey as Function)(targetRoute)
              multiTabStore.removeCachedComponent(customCacheKey)
            } else {
              multiTabStore.removeCachedComponent(componentName)
            }
          }
        })
        ...
        if (activeKey.value === targetKey) {
          const activeRoute = index == routeList.length ? routeList[index - 1] : routeList[index]
          activeKey.value = activeRoute.path
          router.push(activeRoute.fullPath)
        }
      }
    }
    ...
    const handleTabMoreAction = ({ key }: MenuInfo) => {
      ...
      // 更新缓存
      multiTabStore.clearAllCache()
      multiTabStore.routeList.forEach(route => addCachedComponentNames(route.matched, route))
    }
    ...
    const reloadContent = () => {
      ...
      currentRoute.matched.forEach(route => {
        ...
        if (componentName && componentName !== routerLayoutName) {
          ...
          if (route.meta?.cacheStrategy && route.meta?.cacheKey) {
            const customCacheKey = (route.meta.cacheKey as Function)(currentRoute)
            componentKeys.push(customCacheKey)
          } else {
      ...
      componentKeys.forEach(key => {
        multiTabStore.removeCachedComponent(key)
        // 为了刷新，也要从Set中删除
        multiTabStore.cachedComponentNames.delete(key)
      })
      ...
      nextTick(() => {
        ...
        // 添加缓存
        componentKeys.forEach(key => {
          multiTabStore.addCachedComponent(key)
        })
      })
    ...
    // 监听关闭当前tab的事件，支持传入指定路径
    const handleCloseCurrentTab = (targetPath?: string) => {
      const pathToClose = targetPath || activeKey.value
      if (pathToClose && multiTabStore.routeList.length > 1) {
        handleTabEdit(pathToClose, 'remove')
      }
    }

    emitter.on('close-current-tab', handleCloseCurrentTab)
    onUnmounted(() => emitter.off('close-current-tab', handleCloseCurrentTab))
                ...
                default: () => (
                  <>
                    {multiTabStore.routeList.map(route => (
                      <Tabs.TabPane
                        key={route.path}
                        tab={route.query._multiTab || route.meta.name}
