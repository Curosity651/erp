...
const handleDeselect = value => {
  ...
  // 同步移除 selectedRows 中已被取消的行，避免 emit 出脏行
  const keySet = new Set(Array.isArray(value) ? value : [value])
  selectedRows.value = selectedRows.value.filter(row => keySet.has(row[props.dataKey]))
}
